var codeMain = codeMain || {};
codeMain.workspacePath = codeMain.workspacePath || __dirname;
// console.log(codeMain)

var fs = require("fs");
var path = require("path");
// console.log("Asdasdasdfasd")
...
var htmlHandlerScript = function()
{
    var sriptList = [];
    //console.log(classPattern.exec(line));
    var allClassArr = [];
    allClassArr = LayaAirFileTools.getFileList(path.join(codeMain.workspacePath, "src"), ".ts");
    var workspace = codeMain.workspacePath;
    ...
    var htmlcont = fs.readFileSync(path.join(codeMain.workspacePath, "bin", "index.html"), "utf-8");
    ...
    if (oldscript && oldscript.length > 1)
    {
        ...
        fs.writeFileSync(path.join(codeMain.workspacePath, "bin", "index.html"), htmlcont);
    }
}
